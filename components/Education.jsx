...
function EducationCard({data, cta, activeId}) {
    return (
        <div key={data.id} onClick={() => cta(data)} className={`hover:scale-100 transition-all duration-300 p-4 flex items-center gap-4 bg-stone-700 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 backdrop-saturate-100 backdrop-contrast-100 rounded-lg origin-top-left ${(activeId == data.id) ? 'scale-100' : 'scale-95'} group pl-6 relative`}>
            ...
            <div className="w-full">
                <div className="flex justify-between">
                    <p className={`${(activeId == data.id) ? 'text-white' : 'text-gray-500' } group-hover:text-white transition-all duration-400 font-extrabold text-lg`}>{data.degree}</p>
                    <p className={`${(activeId == data.id) ? 'text-gray-400' : 'text-gray-600'} group-hover:text-gray-400 transition-all duration-400 text-base font-semibold`}>
                        {data.startDate} - {data.endDate}
                    </p>
                </div>
                <p className={`${(activeId == data.id) ? 'text-gray-400' : 'text-gray-600'} group-hover:text-gray-400 transition-all duration-400 font-bold text-base`}>{data.institution}</p>
            </div>
    ...
    return (
        ...
        >
            <Card className="border-none bg-stone-700 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 backdrop-saturate-100 backdrop-contrast-100 rounded-lg pl-2.5">
                <CardHeader>
                    <CardTitle className="font-bold">{data.degree}</CardTitle>
                    <CardDescription className="font-bold text-lg">{data.institution}</CardDescription>
                </CardHeader>
                <CardContent>
                    {
                        data.description && (
                            <p className="text-lg font-semibold text-gray-300">{data.description}</p>
                        )
                    }
                    {
...
function OptionalContent({data, title, colorTheme}) {
    ...
    return (
        <div className="my-4">
            <p className="text-xl font-bold mb-1 capitalize">{title}</p> 
            {
                data.map((item, index) => (
                    <div key={index} className="flex items-center gap-3">
                        <span className={`flex h-1.5 w-1.5 rounded-full ${colorClasses[colorTheme]}`} />
                        <p className="text-lg font-bold text-gray-400">{item}</p>
                    </div>
                ))
            }
        </div>
